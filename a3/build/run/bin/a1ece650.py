    ...
    def if_cross(self, l1, l2):
        if not self.if_parallel(l1, l2):
            ...
            if min(x_range) <= x <= max(x_range):
                d = self.get_distance(l1[0][0], l1[0][1], l1[1][0], l1[1][1])
                d1 = self.get_distance(l1[0][0], l1[0][1], x, y)
                d2 = self.get_distance(x, y, l1[1][0], l1[1][1])
                if d1 <= d and d2 <= d:
    ...
    def print_graph(self):
        ...
        if self.edges:
            for i in self.edges[:-1]:
                print("<{0},{1}>,".format(val_list.index(i[0]), val_list.index(i[1])), end='')
            print("<{0},{1}>".format(val_list.index(self.edges[-1][0]), val_list.index(self.edges[-1][1])),
                  end='')
        print("}")
